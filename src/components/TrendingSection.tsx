    ...
    <div>
      <div className="relative w-full h-full group">
        <input
          ...
          className="outline-none peer focus:border-primary focus:border bg-neutral-900/90 w-full h-full rounded-xl py-4 
          pl-14 pr-4" />
        <label 
          htmlFor="searchBox" 
    ...
    <div className="flex flex-col rounded-xl bg-neutral-900 p-4 my-4">
      ...
      <div>
        {Array.from({length: 5}).map((_,i)=> (
          <div key={i} className="hover:bg-white/10 p-4 last:rounded-b-xl first:rounded-t-xl transistion duration-200">
    ...
    <div className="flex flex-col rounded-xl bg-neutral-900 p-4 my-4">
      ...
      <div>
        {Array.from({length: 4}).map((_,i)=> (
          <div 
          key={i} 
          className="hover:bg-white/10 p-4 flex justify-between items-center last:rounded-b-xl first:rounded-t-xl transistion duration-200">
            <div className="flex items-center space-x-2">
              <div className="w-10 h-10 bg-neutral-600 rounded-full"></div>
              <div className="flex flex-col">
                <div className="font-bold text-white">Other User</div>
                <div className="text-gray-500 text-xs">@OtherUser1337</div>
              </div>
            </div>
